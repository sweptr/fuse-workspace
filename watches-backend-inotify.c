...
struct notifyfs_fsevent_struct *evaluate_fsevent_inotify(struct inotify_event *i_event)
{
    ...
    if (i_event->len>0) {
	char eventstring[32];
	struct entry_struct *parent=watch->inode->alias;
	...
	*(notifyfs_fsevent->pathinfo.path+notifyfs_fsevent->pathinfo.len)='\0';
	...
	notifyfs_fsevent->pathinfo.flags=PATHINFOFLAGS_ALLOCATED;
	...
	*/
	...
	entry=find_entry_table(parent, i_event->name, 1);

	if (!entry) {

	    entry=create_entry(parent, i_event->name, NULL);
	...
	if (entrycreated==1) {
		...
		if (inode->st.st_nlink!=st.st_nlink) {

		    fseventmask->move_event|=NOTIFYFS_FSEVENT_MOVE_NLINKS;
		    inode->st.st_nlink=st.st_nlink;
		...
		*/
		...
		if (inode->st.st_mtim.tv_sec<st.st_mtim.tv_sec || (inode->st.st_mtim.tv_sec==st.st_mtim.tv_sec && inode->st.st_mtim.tv_nsec<st.st_mtim.tv_nsec)) {
		...
		if (inode->st.st_ctim.tv_sec<st.st_ctim.tv_sec || (inode->st.st_ctim.tv_sec==st.st_ctim.tv_sec && inode->st.st_ctim.tv_nsec<st.st_ctim.tv_nsec)) {
		...
		if (inode->st.st_atim.tv_sec<st.st_atim.tv_sec || (inode->st.st_atim.tv_sec==st.st_atim.tv_sec && inode->st.st_atim.tv_nsec<st.st_atim.tv_nsec)) {
